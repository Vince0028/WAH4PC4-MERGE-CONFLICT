...
export default function IHOMISSidebar() {
  const pathname = usePathname();
  return (
    <aside className="w-[240px] min-h-screen flex flex-col" style={{ background: 'var(--color-bg-sidebar)' }}>
      <div className="px-5 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-md flex items-center justify-center" style={{ background: 'rgba(37,99,235,0.2)' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
          </div>
          <div>
            <h1 className="text-sm font-semibold text-white leading-tight">iHOMIS</h1>
            <p className="text-[11px] leading-tight" style={{ color: 'var(--color-text-sidebar)' }}>DOH Hospital System</p>
          </div>
        </div>
      </div>
      <nav className="flex-1 p-3 flex flex-col gap-0.5">
        ...
        {navItems.map(item => {
          ...
          return <Link key={item.href} href={item.href} className={`ihomis-sidebar-link ${isActive ? 'active' : ''}`}>{item.icon}<span>{item.label}</span></Link>;
        })}
      </nav>
